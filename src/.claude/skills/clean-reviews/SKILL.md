...
Remove generated review reports from `.reviews/` (in `src/`, alongside the solution). This is the **only** skill that deletes report files — `/standards-review` and `/unit-test-review` never do.
...
2. **Determine which to delete from the user's phrasing:**
   ...
   - "delete the review for ToolKit" / "remove `<name>`" → only the matching file(s). If more than one matches, list them and confirm before deleting.
...
- Never touch `vulnerabilities/` — security reports are committed records and are not cleaned up by this skill.
